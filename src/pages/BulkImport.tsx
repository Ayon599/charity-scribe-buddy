...
import { Upload, Download, FileSpreadsheet, ShieldAlert, Loader2 } from "lucide-react";
import {
  parseRegAndMonthly, toImportPayload, ymToHeader, SHEET_NAME,
  type ParseResult,
} from "@/lib/excelSheet";
import {
  buildRegAndMonthlyWorkbook, downloadWorkbook, monthRange, type ExportMember,
...
export default function BulkImport() {
  ...
  const totals = useMemo(() => {
    ...
    return { members: parsed.rows.length, payments, amount, registration };
  }, [parsed]);
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      const result = parseRegAndMonthly(file.name, buf);
      setParsed(result);
      if (result.rows.length === 0) {
        toast({ title: "Nothing to import", description: "No usable rows were found.", variant: "destructive" });
      }
    } catch (e) {
      toast({ title: "Could not read file", description: safeErrorMessage(e), variant: "destructive" });
    } finally {
      setParsing(false);
    }
  }
  ...
  async function commit() {
    ...
    try {
      const { data, error } = await supabase.rpc("import_reg_and_monthly", {
        p_rows: toImportPayload(parsed.rows) as never,
  ...
  async function exportWorkbook() {
    ...
    try {
      ...
      downloadWorkbook(wb, `Prottoy_Foundation_Reg_and_Monthly_${stamp}.xlsx`);
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Nothing is saved until you review the preview and commit. Re-importing corrects amounts instead of duplicating them.
              </CardDescription>
            </CardHeader>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generates a workbook matching the reference layout: member details, registration fee, then a Date/Amount pair per month.
              </CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Button onClick={() => void commit()} disabled={importing || parsed.rows.length === 0}>
                  {importing && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}
                  Commit import
                </Button>
                <Button variant="ghost" onClick={() => { setParsed(null); setSummary(null); }} disabled={importing}>
                  Discard
                </Button>
                <p className="text-xs text-muted-foreground">
